...
public class TwelveDataAPIService {
	...
	// 800 API CALLS!
	public APIResponse askAPI(String interval, LocalDate startDate, LocalDate endDate)
			throws IOException, InterruptedException, URISyntaxException {
		HttpRequest request = HttpRequest.newBuilder().version(HttpClient.Version.HTTP_1_1)
				.uri(new URI("https://api.twelvedata.com/time_series?symbol=EUR/USD&interval=" + interval
						+ "&start_date=" + formatDateForAPI(startDate) + "&end_date=" + formatDateForAPI(endDate)
						+ "&apikey=e838323726454a30b11fa06845269edb"))
				.GET().header("Content-Type", "application/json; charset=utf-8")
				.header("User-Agent", "JavaHttpClient/1.0").header("Accept", "*/*").timeout(Duration.ofMinutes(1))
				.build();
	...
	static class Value {
		...
		@Override
		public String toString() {
			return "datetime:" + datetime.toString() + ",open:" + open + ",close:" + close + ",high:" + high + ",low:"
					+ low;
		}
	}
}
